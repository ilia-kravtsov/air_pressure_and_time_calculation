...
export const Block: React.FC<BlockType> = ({   title,
    ...
    return (
        <div className={s.fadeForContainer}>
            <Fade direction={columNumber === 14 ? 'left' : 'up'}>
                <div className={s.container}>
                    ...
                    <div className={s.information}>
                        ...
                        {columNumber === 5 && <div
                            className={s.changedBlock}>{`${timeOn ? timeOn?.split(':')[0] + ' ' + 'ч' + ' ' + timeOn?.split(':')[1] + ' ' + 'мин' : '00:00'}`}</div>}
                        {columNumber === 6 && <div>
                            <div>299 атм</div>
                            <div>299 атм</div>
                            <div>299 атм</div>
                        ...
                        {columNumber === 10 &&
                            <div className={s.changedBlock}>{`${T_Exit === 'NaN:NaN' ? '00:00' : T_Exit}`}</div>}
                        {columNumber === 11 && <div className={s.changedBlock}>{`${T_General} мин`}</div>}
                        {columNumber === 12 &&
                            <div
                                className={s.changedBlock}>{`${T_Returning === 'NaN:NaN' ? '00:00' : T_Returning}`}</div>}
                        {columNumber === 13 && <div
                            className={s.changedBlock}>{`${T_Fact_Returning === 'NaN:NaN' ? '00:00' : T_Fact_Returning}`}</div>}
                        {columNumber === 14 && <div className={s.phrases}>
                            <div className={s.subContainer}>
                                ...
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:04')}`}</span>
                                    Звено - посту: прибыли к месту выполнения задания, самочувствие в норме.
                                </div>
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:13')}`}</span>
                                    Пост - звену: возвращайтесь на свежий воздух, доложите обстановку и самочувствие.
                                </div>
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:14')}`}</span>
                                    Звено - посту: возвращаемся на свежий воздух, самочувствие в норме.
                                </div>
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:23')}`}</span>
                                    Пост - звену: доложите обстановку и самочувствие.
                                </div>
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:24')}`}</span>
                                    Звено - посту: звено возвращается на свежий воздух самочувствие в норме
                                </div>
                                <div>
                                <span
                                    className={s.changedBlock}>{`${timeCount(timeOn ? timeOn : '00:00', '00:30')}`}</span>
                                    Выход звена
                                </div>
                            </div>
                        </div>}
